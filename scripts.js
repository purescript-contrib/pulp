...
// updated.
//
// We set the PSVM_PREFIX to ensure that psvm only installs stuff in the
// current directory.
function getSubcommandEnv() {
  var env = Object.assign({}, process.env);
  env.PSVM_PREFIX = path.join(__dirname, ".psvm");
  prependPath(env, path.join(env.PSVM_PREFIX, "current", "bin"));
  return env;
}
...
function ensurePscVersionsInstalled() {
  ...
  mkdirp.sync(path.join(subcommandEnv.PSVM_PREFIX, 'versions'));
  ...
  versions.forEach((version) => {
    if (!installedVersions.includes(version)) {
      console.log("Installing ", version, " via psvm...");
